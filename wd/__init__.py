...
def httpdo(url, method='GET', data=None):
    ...
    if DEBUG:
        print("Shell: curl -X {method} -d '{var}' '{url}'".format(method=method.upper(), data=data or '', url=url))
...
class HTTPClient(object):
    ...
    def fetch(self, method, url, data=None):
        ...
        # return httpdo(urljoin(self.address, url), method, var)
...
class Session(object):
    ...
    def swipe_down(self):
        w, h = self.window_size()
        return self.swipe(w/2, 100, w/2, h)
...
class Selector(object):
    ...
    def _fix_xcui_type(self, s):
        if s is None:
            return
        re_element = '|'.join(event_type.ELEMENTS)
